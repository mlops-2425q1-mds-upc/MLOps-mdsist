...
def setup_environment(sample_data):
    ...
    with tempfile.NamedTemporaryFile(delete=False) as temp_train, \
    ...
    yield (train_path, test_path, processed_train_path, processed_validation_path, processed_test_path, params_path)
...
def test_data_preprocessing(setup_environment):
    ...
    train_path, test_path, processed_train_path, processed_validation_path, processed_test_path, params_path = setup_environment

    # Patch to replace the location of params.yaml in the main function
    with patch("mdsist.preprocessing.open", lambda f, *args, **kwargs: open(params_path, *args, **kwargs) if f == "params.yaml" else open(f, *args, **kwargs)):
    ...
    assert len(train_df) + len(validation_df) + len(test_df) == 15, "Total number of records is incorrect."
...
def test_main_with_invalid_data(setup_environment):
    ...
    with patch("mdsist.preprocessing.open", lambda f, *args, **kwargs: open(params_path, *args, **kwargs) if f == "params.yaml" else open(f, *args, **kwargs)):
        ...
        with pytest.raises(FileNotFoundError):
            main(invalid_train_path, test_path, processed_train_path, processed_validation_path, processed_test_path)
